...
\begin{questions}
    ...
    \begin{parts}
        ...
        \begin{proof}
            Let $X\in\R^{n x d}, y\inR^d$ be such that the $i-th$ row of $X$ is $x_i$ and $y_i$ respectively. Then
            \begin{align*}
                \frac{1}{n}\sum_{i = 1}^n (y_i - a^Tx_i)^2 &= \frac{1}{n}\norm{y - Xa}^2 \\
                &= \frac{1}{n}\paren{y^Ty - 2a^TA^Ty + a^TA^TAa} \\
                \nabla_a f &= \frac{1}{n}\paren{2A^TAa - 2A^Ty} = \frac{2}{n}\paren{A^TAa - A^Ty} \\
                \nabla_a^2 f &= \frac{2}{n}A^TA.
            \end{align*}
            Thus we are done if we can show that $A^TA$ is PSD. Consider $v\in\R^d$. We compute 
            $$v^TA^TAv = (Av)^TAv = \norm{Av}^2 \geq 0.$$
            Thus $\nabla_a^2 f \succeq 0$, and $f$ is convex.
        \end{proof}
        ...
        \begin{proof}
            Let $S = \sum_{i = 1}^n x_i^p$, and note that $S > 0$. We first directly compute the gradient of $f$:
            $$\frac{\del f}{\del x_k} = \frac{1}{p}S^{1/p - 1}\cdot px_k^{p - 1} = S^{1/p - 1}x_k^{p - 1}.$$
            Denote $v = (x_1^{p - 1}, \hdots x_n^{p - 1})^T$ for convenience, and rewrite $\nabla f = S^{1/p - 1}v$.

            Now compute the hessian.
            \begin{align*}
                \frac{\del f}{\del x_j \del x_k} &= \frac{1}{p}(1 - p)S^{1/p - 2}\cdot px_j^{p - 1}x_k^{p - 1} + (p - 1)x_k^{p - 2}S^{p - 1} \\
                &= (1 - p)S{1/p - 2}x_j^{p - 1}x_k^{p - 1} + (p - 1)x_k^{p - 2}S^{1/p - 1}.
            \end{align*}
            Again for notational ease, set $D = \text{diag}\paren{x_1^{p - 2}, \hdots x_n^{p - 2}}^T$. Rewrite the hessian as
            $$\nabla^2 f = (1 - p)S^{1 /p - 2}vv^T + (p - 1)S^{1/p - 1}D = (1 - p)S^{1/p - 2}(vv^T - SD).$$
        ...
        \begin{proof}
            Define $T = \cbrac{t : X + tV \in\dom f}$ and
            $$g(t) = f(X + tV) = \tr{(X + tV)^\inv}.$$
            It is sufficient to show that $g(t)$ is convex over $T$. Let $X + tV = tQ\Lambda Q^T$ and $\lambda_i > 0$ an eigenvalue for
            $X + tV$. 
            \begin{align*}
                g(t) &= \tr{(X + tV)^\inv} \\
                &= \tr{X^{-1/2}(I + tX^{1/2}VX^{1/2})^\invX^{-1/2}} \\
                &= \tr{X^\inv(I + tX^{1/2}VX^{1/2})} \\
                &= \tr{X^{-1}Q(I + tX^{1/2}\Lambda X^{1/2})Q^T} \\
                &= \tr{Q^TX^\inv Q(I + tX^{1/2}\Lambda X^{1/2})} \\
                &= \sum_{i = 1}^n \paren{Q^TX^\inv Q}_{ii}(1 + t\lambda_i).
            \end{align*}
            Note that $h_i(t) = 1 + t\lambda_i > 0$ is obviously convex. Consider the case when $(Q^TX^\inv Q)_{ii} \geq 0$, 
            then we have
            $$g(t) = \sum_{i = 1}^n (Q^TX^\inv Q)_{ii} h_i(t),$$
            which is the non-negative weighted sum of convex functions, which is convex. Thus it is sufficient to show $Q^TX^\inv Q \geq 0$.
            $X \succ 0$, let $X = USU^T$ for an orthogonal matrix $U$ and diagonal matrix $S$. Then
            \begin{align*}
                \paren{Q^TX^\inv Q}_{ii} &= \paren{Q^TUSU^TQ}_{ii} \\
                &= \paren{(Q^Tu)S(Q^TU)^T}_{ii}.
            \end{align*}
            Let $H = Q^TU$. We can further simplify the above equation into
            $$\paren{(Q^Tu)S(Q^TU)^T}_{ii} = \paren{HSH^T}_{ii},$$
            and since $S$ is a diagonal matrix of eigenvalues for a PD matrix, $HSH^T$ is also PD. We conclude that $(HSH^T)_{ii} > 0$, and by
            extension $g(t)$ is convex.
        \end{proof}
    \end{parts}
    ...
    has zero gradient.)
    \begin{proof}
        By definition we know that the conjugate function for $f$ is defined as
        $$f^*(Y) = \sup_{X\succ 0}\paren{\tr{XY} - \tr{X^\inv}}.$$
