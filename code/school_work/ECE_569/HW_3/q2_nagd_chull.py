...
'''
    ...
    for i in 1 to MAX_ITER do:
        ...
        u_new = project_to_simplex(u_temp)
        v_new = project_to_simplex(v_temp)
...
plt.title("Iteration vs Objective Value (Nesterov C-Hull)")
plt.grid(True)
plt.tight_layout()
...
plt.xlabel("Time (ms)")
plt.ylabel("Objective Value")
plt.title("Time vs Objective Value (Nesterov C-Hull)")
plt.grid(True)
plt.tight_layout()
plt.show()